-- Migration: Fix gen_salt type cast issue in create_user_with_role
-- Purpose: Add explicit integer cast to gen_salt cost parameter
-- Issue: PostgreSQL couldn't resolve gen_salt(text, unknown) - needs gen_salt(text, integer)
-- Date: 2025-10-30

-- Drop the function to recreate with fixed gen_salt call
DROP FUNCTION IF EXISTS public.create_user_with_role CASCADE;

-- Recreate with explicit type cast
...
'Create user with temporary password. Uses explicit type cast for gen_salt cost parameter. Looks up tenant from user_tenant_access table (more reliable than JWT). Admin must immediately send password reset email for security.';
...
BEGIN
  ...
  ELSE
    RAISE NOTICE 'SUCCESS: Function create_user_with_role created with type cast fix';
  END IF;
END $$;
