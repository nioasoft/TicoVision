-- Migration: Fix collection_dashboard_view - Add security_invoker
-- Date: 2025-11-04
-- Purpose: Fix SECURITY DEFINER ERROR by adding security_invoker=true
--
-- Supabase Advisor Issue: security_definer_view
-- Severity: ERROR
-- Impact: View was running with creator permissions instead of querying user
--
...
Uses security_invoker=true to respect RLS policies and prevent SECURITY DEFINER warning.';

-- ============================================================================
-- Verification Query
-- ============================================================================
-- Run this to verify security_invoker is set:
--
-- SELECT relname, reloptions
-- FROM pg_class
-- WHERE relname = 'collection_dashboard_view'
--   AND relkind = 'v';
--
