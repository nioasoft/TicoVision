-- ============================================================================
-- Migration: shaagat_initial_filter_view v3 — submission deadline columns
-- Date: 2026-05-10
-- Purpose: Surface the latest submission's deadline data on the unified
-- dashboard so the new "context info" cell can show the nearest deadline
-- (advance / determination / full payment) inline per row.
--
-- Backwards compatible: same view name, all existing columns preserved.
-- New columns appended at the end.
-- ============================================================================

-- Drop first because we are reordering columns (PG doesn't allow that via CREATE OR REPLACE).
DROP VIEW IF EXISTS shaagat_initial_filter_view;

CREATE VIEW shaagat_initial_filter_view
...
SELECT
  ...
  -- Tax submission (with deadlines, new in v3)
  ls.submission_id,
...
COMMENT ON VIEW shaagat_initial_filter_view IS
  'Active clients + their latest Shaagat HaAri lifecycle data (eligibility, accounting form, calculation, submission with deadlines, retainer). Backs the unified dashboard.';
