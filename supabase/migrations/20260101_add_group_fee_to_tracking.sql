-- ================================================
-- Migration: Add Group Fee Calculation Data to Fee Tracking
-- Date: 2026-01-01
-- Description:
--   Extend get_fee_tracking_data() to return group fee calculation data.
--   This allows the fee tracking UI to display:
--   - Group audit fees (before and with VAT)
--   - Group bookkeeping fees (before and with VAT)
--   - Group calculation status
--   - Group letter sent timestamp
-- ================================================
...
RETURNS TABLE (
  ...
  -- NEW: Group fee calculation data
...
BEGIN
  ...
  -- NEW: Get group letter (sent to the group, not individual)
  LEFT JOIN LATERAL (
    SELECT
      ggl_inner.id,
      ggl_inner.sent_at
...
FIX (2025-12-31): Now correctly identifies letters sent to groups.
NEW (2026-01-01): Returns group fee calculation amounts and status.';
