-- ════════════════════════════════════════════════════════════════════
-- Shaagat HaAri: only `tax_coding_status = 'regular'` clients appear in
-- the unified view. Previously NULL passed through, but NULL means "not
-- yet classified" — no safer than 'zero' for a revenue-based grant
-- module that needs accurate Form 1214 data.
-- ════════════════════════════════════════════════════════════════════
...
CREATE OR REPLACE VIEW public.shaagat_initial_filter_view AS
 ...
 SELECT c.id AS client_id,
  ...
  WHERE c.status = 'active'::text
    ...
    AND c.tax_coding_status = 'regular';
