/**
 * GroupFeeHistoryCard - Group fee calculations across years
 */
...
interface GroupFeeHistoryCardProps {
...
export function GroupFeeHistoryCard({ feeCalculations }: GroupFeeHistoryCardProps) {
  return (
    <Card className="rounded-xl">
      <CardHeader className="pb-2">
        <CardTitle className="text-sm font-medium flex items-center gap-2">
          <Calculator className="h-4 w-4 text-muted-foreground" />
          חישובי שכ&quot;ט קבוצתיים
        </CardTitle>
      </CardHeader>
      <CardContent className="p-3 pt-0">
        {feeCalculations.length === 0 ? (
          <p className="text-sm text-muted-foreground text-center py-4">אין חישובי שכ&quot;ט קבוצתיים</p>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b text-xs text-muted-foreground">
                  ...
                  <th className="text-right pb-2 pe-3 font-medium">ביקורת</th>
                  <th className="text-right pb-2 pe-3 font-medium">הנה&quot;ח</th>
                  ...
                  return (
                    <tr key={fee.id} className="border-b last:border-0 hover:bg-muted/30">
                      ...
                      <td className="py-2 pe-3 tabular-nums" dir="ltr">
                        {fee.audit_final_amount_with_vat ? formatILS(fee.audit_final_amount_with_vat) : '—'}
                      </td>
                      <td className="py-2 pe-3 tabular-nums" dir="ltr">
                        {fee.bookkeeping_final_amount_with_vat ? formatILS(fee.bookkeeping_final_amount_with_vat) : '—'}
                      </td>
                      <td className="py-2 pe-3 tabular-nums font-medium" dir="ltr">
                        {fee.total_final_amount_with_vat ? formatILS(fee.total_final_amount_with_vat) : '—'}
                      </td>
